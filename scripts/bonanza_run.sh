...
time python pysrc/experiments/prostheticexp.py s'$s' '$a$aval' prosthetic_experiment '$alg' experiment '$var' > txt/'$alg'-s'$s'-'$a$aval'-'$var'.txt

if [[ '$aval' < "3" ]]
then
	./sh/'$alg'-s'$s'-'$a$(($aval+1))'-'$var'.sh
	echo 'next action'
elif [[ '$s' < "4" ]]
then
    ./sh/'$alg'-s'$(($s+1))'-'$a'1-'$var'.sh
	echo 'next person'
#else
#    ./sh/'$alg'-s1-'$a'1-'$(($var+1))'.sh
#    echo 'next config'
fi
echo done
' > sh/$alg-s$s-$a$aval-$var.sh                           # make a script to run as a process on westgrid

chmod +x sh/$alg-s$s-$a$aval-$var.sh

((var=var+1))                   # increment the config number to move to the next file
...
do
for a in a                                         # for all actions in a session
do
s=1
